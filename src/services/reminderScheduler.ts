...
class ReminderScheduler {
  ...
  private checkScheduledItems() {
    ...
    // 5. Check Habits with optional reminder alarm enabled
    const habits = db.getHabits();
    const habitLogs = db.getHabitLogs();
    const todayWeekdayIdx = db.getPersianWeekdayIndex(todayIso);
    ...
    for (const habit of habits) {
      ...
      const targetDays = Array.isArray(habit.targetDays) && habit.targetDays.length > 0
        ? habit.targetDays
        : [0, 1, 2, 3, 4, 5, 6];
      if (!targetDays.includes(todayWeekdayIdx)) continue;

      // If already marked completed today, do not trigger alarm
      const isDoneToday = habitLogs.some((l) => l.habitId === habit.id && l.date === todayIso && l.completed);
      if (isDoneToday) continue;

      const status = checkDueStatus(todayIso, habit.reminderTime);
      const alarmKey = `habit_${habit.id}_${todayIso}_${habit.reminderTime}`;
      if (status === 'past') {
        if (!notified.has(alarmKey)) {
          this.markNotified(alarmKey);
        }
        ...
        if (!notified.has(alarmKey)) {
          ...
          this.triggerAlarm({
            ...
            subtitle: habit.description || `ساعت موعد: ${habit.reminderTime}`,
            type: 'habit',
