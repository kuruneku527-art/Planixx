...
const DEFAULT_SETTINGS: UserSettings = {
  theme: 'dark',
  persianDigits: false, // Default to English digits as requested
  calendarType: 'jalali',
...
class DatabaseService {
  ...
  getSettings(): UserSettings {
    const raw = this.get<UserSettings>(STORAGE_KEYS.SETTINGS, DEFAULT_SETTINGS);
    const settings = { ...DEFAULT_SETTINGS, ...raw };
    if (typeof window !== 'undefined') {
      const explicitChoice = localStorage.getItem('planner_explicit_digits_choice');
      if (!explicitChoice) {
        settings.persianDigits = false;
      }
    }
    return settings;
  }
  ...
  updateSettings(partial: Partial<UserSettings>): UserSettings {
    if (typeof window !== 'undefined' && 'persianDigits' in partial) {
      localStorage.setItem('planner_explicit_digits_choice', 'true');
    }
