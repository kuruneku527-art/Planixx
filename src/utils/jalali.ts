...
export function toPersianDigits(input: string | number | null | undefined, force = false): string {
  if (input === null || input === undefined) return '';
  if (!force) {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('planner_settings_v1');
        if (stored) {
          const parsed = JSON.parse(stored);
          if (parsed?.persianDigits === true) {
            return String(input).replace(/[0-9]/g, (w) => PERSIAN_DIGITS_MAP[w] || w);
          }
        }
      }
    } catch {}
    // By default, display all digits in standard English digits across the entire app
